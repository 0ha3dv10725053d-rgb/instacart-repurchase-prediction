...
再購入率を含むモデルは予測精度は高い一方で、行動変数との共線性により係数解釈が難しくなるため、事業施策への落とし込みを重視して再購入率を除外したモデルから事業における施策を考えました。

 # 施策
 総購入数が多いユーザーおよび部門別総購入数が多いユーザーが再購入する傾向がみられた。また、購入スパンが伸びると再購入率が下がることが確認された。
 そのため、初期段階に購入スパンを短くするためのクーポン等の施策を部門ごとに実施することが効果的であることが示唆された。
